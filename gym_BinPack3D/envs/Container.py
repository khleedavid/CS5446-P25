...
class Container(object):
    ...
    def update_x_most(self, box):
        # Update the x-most value based on the placed box's position and dimensions
        new_x_most = box.x + box.dx
        if new_x_most > self.x_most:
            self.x_most = new_x_most
    ...
    def get_usage_ratio(self):
        vo = sum([box.dx * box.dy * box.dz for box in self.boxes])
        if self.x_most == 0:
            mx = 1 * self.dy * self.dz
        else:
            mx = self.x_most * self.dy * self.dz
        ratio = vo / mx
        assert ratio <= 1.0
        return ratio
